"""
mesonet implementation but with extra convolutional layer and 3-layer MLP. The hyperparameters are hardcoded
to what they are in the final report. This is the model that provided 100% accuracy on the validation set.
"""
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(3, 16, 3, padding=1),  # same conv
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.Conv2d(16, 32, 3, padding=1),  # same conv
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(2),  # 256 -> 128
            nn.Conv2d(32, 64, 3, padding=1),  # same conv
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(2),  # 128 -> 64
            nn.Conv2d(64, 128, 3, padding=1),  # same conv
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d(2),  # 64 -> 32
            nn.Conv2d(128, 256, 3, padding=1),  # same conv
            nn.BatchNorm2d(256),
            nn.ReLU(),
...
def train(
    ...
    train_progress = []
    pre_epoch_acc = 0.99  # start saving models only after achieving this accuracy
    for epoch in range(num_epochs):  # loop over the dataset multiple times
        ...
        if accuracy > pre_epoch_acc:
            # save model if it's good
            pre_epoch_acc = accuracy
            torch.save(
                net.state_dict(),
                "/home/username/MAIN/dev/CS481/final_project/202610_csci581_project_gel_han_tri/code/task4/main/models/"
                + run_name
                + "_epoch_"
                + str(epoch + 1)
                + ".pth",
...
def main():
    ...
    transformation = transformations.flip_blur_cj
    ...
    # make dataframe and save output
    df = pd.DataFrame(
        train_progress,
